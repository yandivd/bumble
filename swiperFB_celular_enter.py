...
def send_email(number):
    ...
    # Obtener la fecha y hora actual en la zona horaria del Pacífico
    date = datetime.now(pacific_time_zone).strftime("%B-%d-%Y %H:%M")
    msg = MIMEMultipart()
...
def coneccion_driver_extension_location():
    ...
    with open('cookies_fb_adil_20_03.json', 'r') as cookie_file:
    ...
    numero1 = int(random.uniform(150, 170))
    for x in  range(numero1):
        ...
        codigo_fuente = driver.page_source
        soup = bs(codigo_fuente, 'html.parser')

        # Buscar la cadena
        if "You're All Caught Up" in soup.text:
            send_email(numero1)
        else:
            continue
