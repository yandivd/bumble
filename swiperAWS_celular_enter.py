...
def replace_domain(cookie):
    if 'us1.bumble.com' in cookie['domain']:
        cookie['domain'] = cookie['domain'].replace('us1.bumble.com', '.bumble.com')
    return cookie

def change_same_site_to_lax(cookie):
    cookie['sameSite'] = 'Lax'
    return cookie

# region valid cookies
    ...
    try:
        with open('cookies_test.json', 'r') as cookie_file:
            ...
            for cookie in cookies:
                ...
                c = replace_domain(change_same_site_to_lax(cookie))
                driver.add_cookie(c)
